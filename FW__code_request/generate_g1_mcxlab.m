function [tau,g1]=generate_g1_mcxlab(cfg,detpos,tau,disp_model,DV,lambda)
%
%   [tau,g1]=generate_g1(detpos,tau, disp_model, DV, lambda)
%
...
%
%   input:
%       cfg:        the cfg structure used as input to mcxlab
%       detpos:     the detpos structure output by mcxlab
...
%
if nargin<6, lambda=785; end
if nargin<5, DV=1e-7; end
if nargin<4, disp_model='brownian'; end
if nargin<3, tau=logspace(-7,-1,200); end

if ~isfield(detpos,'mom'),
    fprintf('Detpos structure does not contain momentum transfer information \n');
    g1=-1;
    return;
end    
...
mua=cfg.prop(2:end,1)';


g1=zeros(detnum,length(tau));

for I=1:detnum,
    idx= find(detpos.detid==I);     
    fprintf('Processing detector %.0f: %.0f photons\n',I,length(idx));

    for J=1:length(tau),
        eval(disp_str);
        g1(I,J)=sum(exp(-(k0.^2.*rmsdisp/3)*detpos.mom(idx,:)'-mua*detpos.ppath(idx,:)'));
    end
    g1_norm=sum(exp(-mua*detpos.ppath(idx,:)'));
    g1(I,:)=g1(I,:)./g1_norm;
end